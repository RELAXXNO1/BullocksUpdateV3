...
const AccountDetailsModal: React.FC<AccountDetailsModalProps> = ({ isOpen, onClose }) => {
  ...
  console.log("AccountDetailsModal AuthContext:", authContext); // Debug log context value
  const { user, deleteAccount, logout } = authContext || useAuth(); // Use context, fallback to hook if context is not available
  console.log("AccountDetailsModal user:", user); // Debug log user value
  console.log("AccountDetailsModal user?.email:", user?.email); // Debug log user email
